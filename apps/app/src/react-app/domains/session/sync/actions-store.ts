...
const uploadAttachmentToFastMCP = async (file: File): Promise<string> => {
...
export function createSessionActionsStore(options: {
  ...
  const attachmentToFilePart = async (attachment: ComposerAttachment): Promise<PartInput> => {
    ...
    // Wilsch patch (#1834): prefix with SENTINEL_PREFIX so the render-filter
    // patch hides this text from Susan's bubble. This path is the actions-store
    // emission site (non-user-send code paths); session-route.tsx mirrors the
    // sentinel + adds a file-chip part for the user-send path.
    return {
      type: "text",
      text:
        SENTINEL_PREFIX +
